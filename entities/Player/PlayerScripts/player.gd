...
@export var _player_health: int = 100
...
@onready var _player_ui: PlayerUI = $player_ui

signal was_hit(new_health)
...
func set_enemy_health(enemy: Enemy) -> void:
	_player_ui.set_up_enemy_health(enemy)

func give_damage(damage: int) -> void:
	_player_health -= damage
	if _player_health <= 0:
		get_tree().reload_current_scene()
	emit_signal("was_hit",_player_health)
